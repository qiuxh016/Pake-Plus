...
启用系统级文本剪贴板历史。该功能默认关闭，因此原有 Pake 应用不会启动后台剪贴板监听器，也不会创建历史数据库。历史仅保存在应用配置目录；本地历史面板可以配置短文本、密码样式、银行卡样式和来源应用过滤。

```shell
--clipboard
```

Windows/Linux 使用 `Ctrl+Shift+V`，macOS 使用 `Cmd+Shift+V` 打开历史面板。平台能够提供来源信息时，每条记录会显示对应应用名称。启用系统托盘后，也可从托盘菜单的 **Clipboard History** 入口打开。

#### [clipboard-max]

设置剪贴板历史的最大记录数，允许范围为 `500` 到 `5000`，默认值为 `2000`。即使没有启用 `--clipboard`，该值仍会写入生成配置，但在剪贴板历史启用前不会生效。

同一应用重新打包后，新包中的容量上限会覆盖旧值，同时保留用户本地配置的隐私过滤规则。
